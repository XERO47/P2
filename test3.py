...
class SalesDatasetTester:
    def __init__(self, api_url="http://localhost:8000/api/"):
        ...
        self.test_question = """Analyze the attached `sample-sales.csv`.

Return a JSON object with the following keys:
- `total_sales`: A number representing the sum of all sales.
- `top_region`: A string for the region with the highest total sales.
- `day_sales_correlation`: A number for the correlation between the day of the month and sales.
- `bar_chart`: A base64 PNG data URI string of a bar chart showing total sales by region. The bars must be blue.
- `median_sales`: A number for the median sales amount across all orders.
- `total_sales_tax`: A number for the total sales tax, calculated as 10% of total sales.
- `cumulative_sales_chart`: A base64 PNG data URI string of a line chart showing cumulative sales over time. The line must be red.
"""
